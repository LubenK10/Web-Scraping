...
var productNodes = doc.DocumentNode.SelectNodes("//div[@class='item']");
if (productNodes != null)
{
    foreach (var productNode in productNodes)
    {
        ...
        // Product Name
        var productNameNode = productNode.SelectSingleNode(".//h4/a");
        if (productNameNode != null)
        {
            productInfo["productName"] = WebUtility.HtmlDecode(productNameNode.InnerText.Trim());
        }
        ...
        if (priceNode != null)
        {
            ...
            {
                var priceText = priceNode.SelectSingleNode(".//span[@class='dollars']").InnerText.Trim() +
            ...
            {
                var priceText = priceNode.SelectSingleNode(".//span[@class='dollars over500']").InnerHtml;
                productInfo["price"] = priceText;
            }
        ...
        // Rating (normalized to be out of 5)
        var ratingAttribute = productNode.GetAttributeValue("rating", null);
        if (decimal.TryParse(ratingAttribute, NumberStyles.Number, CultureInfo.InvariantCulture, out var rating))
        {
            if (rating > 5) 
            {
                rating /= 2;
            } // Normalize rating to be between 0 and 5
            productInfo["rating"] = rating.ToString();
        }
